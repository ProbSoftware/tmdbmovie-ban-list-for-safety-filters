...
namespace ProbSoftwareMiner
{
    internal class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            {
                Console.WriteLine("\n[HATA] TMDB_API_KEY bulunamadı! Lütfen GitHub Secrets'a veya bilgisayarına API key'i ekle.");
                return;
            }

            // Dosya yolları GitHub Actions ana dizinine (repo köküne) ayarlandı.
            ...
            // --- 0. FAZ: HAFIZAYI (STATE) YÜKLEME ---
            Console.WriteLine("\n--- [FAZ 0] HAFIZA YÜKLENİYOR ---");
            if (File.Exists(dataPath))
            {
                Console.WriteLine("📦 Eski veritabanı (filter_data.json) bulundu, belleğe yükleniyor...");
                try
                {
                ...
                catch (Exception ex) { Console.WriteLine($"Veritabanı okunurken hata: {ex.Message}"); }
            }

            if (File.Exists(statePath))
            {
                Console.WriteLine("📦 İşlenmiş keyword geçmişi (islenen_keywords.json) yükleniyor...");
                try
                {
            ...
            {
                if (state.IslenenYetiskinKeywords.Contains(kwId))
                {
                    Console.WriteLine($"[ATLANDI] YETİŞKİN KW:{kwId} zaten daha önce taranmış.");
                    continue;
                }

                await FetchData(kwId, "movie", banliFilmler, null, "🔞 YET-FILM");
                await FetchData(kwId, "tv", banliDiziler, null, "🔞 YET-DIZI");

                // İşlendi olarak işaretle ve anında kaydet (GitHub yarıda keserse diye)
            ...
            {
                if (state.IslenenHassasKeywords.Contains(kwId))
                {
                    Console.WriteLine($"[ATLANDI] HASSAS KW:{kwId} zaten daha önce taranmış.");
                    continue;
                }

                await FetchData(kwId, "movie", hassasFilmler, banliFilmler, "⚠️ HAS-FILM");
                await FetchData(kwId, "tv", hassasDiziler, banliDiziler, "⚠️ HAS-DIZI");
        ...
        static async Task<int> FetchData(int keywordId, string type, HashSet<int> targetSet, HashSet<int> upperSet, string logTag)
        {
            int page = 1;
            int totalPages = 1;
            ...
            try
            {
                do
                {
                    ...
                    Console.Write($"\r[ISTEK] {logTag} | KW:{keywordId} | Sayfa:{page}/{totalPages} | Yeni Eklenen ID:{addedInThisKeyword}     ");
                    page++;
                    await Task.Delay(80); // API Rate Limit koruması
        ...
        static async Task FinalizeAndExportJson(string currentPath)
        {
            ...
            Console.WriteLine("------------------------------");
            Console.WriteLine($"✅ Veritabanı başarıyla güncellendi: {currentPath}");
        }
